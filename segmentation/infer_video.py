    ...
    while cap.isOpened():
        ...
        if result.masks is not None:
            ...
            for i, polygon in enumerate(polygons):
                ...
                # Calculate the actual pixel area of the mask
                pixel_area = int(cv2.contourArea(polygon.astype(np.int32))) if len(polygon) > 0 else 0
                
                # Print pixel data to terminal (only every 30 frames to avoid spam)
                if frame_count % 30 == 0:
                    print(f"  Detected: {label_text} | Area: {pixel_area} px | Vertices: {len(polygon)}")
                    if len(polygon) > 0:
                ...
                if len(polygon) > 0:
                    ...
                    cv2.polylines(annotated_frame, [pts], isClosed=True, color=(0, 255, 255), thickness=1)
                ...
                if len(polygon) > 0:
                    ...
                    if M["m00"] != 0:
                        ...
                        # Draw the pixel area data right below it!
                        pixel_text = f"Area: {pixel_area} px | Vertices: {len(polygon)}"
